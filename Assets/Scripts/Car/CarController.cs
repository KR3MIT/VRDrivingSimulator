...
public class CarController : MonoBehaviour
{
    ...
    void CheckInput()
    {
        ...
        if (gasPedal.ReadValue<float>() > 0.02f)
        {
            gasInput += gasPedal.ReadValue<float>();
        }
        if (brakePedal.ReadValue<float>() > 0.02f)
        {
            gasInput -= brakePedal.ReadValue<float>();
        }
        ...
        if (steer.ReadValue<float>() != 0)
        {
            steeringInput += steer.ReadValue<float>();
        }
